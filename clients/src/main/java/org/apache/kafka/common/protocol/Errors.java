...
public enum Errors {
    ...
    // 这里很巧妙，因为经常碰到这样的需求：根据某个值，获取该值对应的具体的枚举常量，我经常是定义一个方法，循环比较定义的枚举常量中这个字段的值，如果相等就返回该枚举类，
    // 但是这里提前声明一个静态的HashMap，并且在静态代码快中往该Map中放入该字段和枚举常量的映射，这样代码运行时获取时就比较快。
    static {
    ...
    /**
     * 这里很巧妙，因为Exception最好在需要的时候生成，如果只是想获取错误码，不需要异常，最好生成异常，这里通过异常 builder 的apply方法生成异常
     * 生成异常的提示信息是defaultExceptionString，而Error这个枚举类中没有defaultExceptionString这个成员变量，但是Error类中声明枚举常量时，
     * 第二个变量是defaultExceptionString，由此可见，声明枚举常量时，需要的变量是有构造函数决定的，而不是成员变量。
     * @param code
     * @param defaultExceptionString
     * @param builder 这个是函数接口 Function<String, ApiException>，可以看上面的声明枚举常量，都是...Exception::new，这里用到了“::”这个符号后面引用的new方法，
     *                ...::new的返回就是Function<T,R>，这个java8“::”的特性，返回的Function<T,R>的apply方法中调用就是“::”这个符号后面引用的方法
     */
