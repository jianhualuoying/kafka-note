...
public class KafkaChannel implements AutoCloseable {
    ...
    public void setSend(Send send) {
        ...
        this.send = send;
        // 在方法PlaintextChannelBuilder.buildChannel(String, java.nio.channels.SelectionKey, int, org.apache.kafka.common.memory.MemoryPool, org.apache.kafka.common.network.ChannelMetadataRegistry)
        // 中可以看到PlaintextTransportLayer是怎么构建的，在kafka.network.Processor.configureNewConnections方法中，注册读事件时，构建的TransportLayer

        // TransportLayer封装了SelectionKey和SocketChannel（构建的时候分别赋值创建客户端连接对应的SelectionKey和SocketChannel），方法addInterestOps其实就是SelectionKey添加写事件
    ...
    public long read() throws IOException {
        if (receive == null) {
            // 创建网络读取类，NetworkReceive类有读缓冲区org.apache.kafka.common.network.NetworkReceive.buffer（ByteBuffer），通过 jdk nio 的SocketChannel读取网络数据，
            // 而且SocketChannel是通过org.apache.kafka.common.network.KafkaChannel.transportLayer获取的
    ...
    public NetworkReceive maybeCompleteReceive() {
        // org.apache.kafka.common.network.KafkaChannel#read方法中，把读取到的数据放到了receive.buffer中，receive#payload方法就是获取receive.buffer
