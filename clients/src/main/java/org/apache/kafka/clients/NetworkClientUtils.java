...
public final class NetworkClientUtils {
    ...
    public static ClientResponse sendAndReceive(KafkaClient client, ClientRequest request, Time time) throws IOException {
        try {
            ...
            while (client.active()) {
                // 这里才是真正的发送数据，方法中调用org.apache.kafka.common.network.Selector#poll方法完成数据读写
                // 最后写数据时，是通过org.apache.kafka.common.network.KafkaChannel.send#writeTo方法 -> java.nio.channels.GatheringByteChannel#write(java.nio.ByteBuffer[])方法，完成真正数据的写
                // 其中，GatheringByteChannel是jdk nio接口负责通道数据的写，ScatteringByteChannel同样是jdk nio接口，负责通道数据的读取，TransportLayer是kafka自定义的接口，继承了前面两个接口，
                // TransportLayer是真正底层数据的读写类，他直接操作jdk nio 的socketChannel。
