...
public class NetworkClient implements KafkaClient {
    ...
    private void doSend(ClientRequest clientRequest, boolean isInternalRequest, long now) {
        ...
        // 获取请求构造类，不同类型的请求的AbstractRequest.Builder构造逻辑不同
        ...
        try {
            ...
            // 区分不同类型的请求，主要是通过 构建请求中的 org.apache.kafka.common.protocol.ApiKeys 字段区分，这里builder.build方法就是构建请求
            // clientRequest有点像请求的工具类，保存所有的请求数据，但是，真正操作的是请求体和请求头，请求体是通过 builder.build方法构造，请求头是通过clientRequest.makeHeader方法构造
    ...
    private void doSend(ClientRequest clientRequest, boolean isInternalRequest, long now, AbstractRequest request) {
        ...
        // 这里并没有真正的发送请求，只是构造NetworkSend对象（对象中序列化好的数据）
        ...
        // 通过 org.apache.kafka.common.network.Selector 的 send 方法，发送数据（实际上是把序列化后的数据放到org.apache.kafka.common.network.KafkaChannel.send字段上，并没有真正发送）
        // TODO 不明白 selector 中 org.apache.kafka.common.network.KafkaChannel是何时构建并注册到selector上的，这里只是往selector中SelectKey上注册写事件
        selector.send(send);
    }
