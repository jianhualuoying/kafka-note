 ...
 * @param securityProtocol 监听器使用的安全协议。Kafka 支持 4 种安全协议，分别是PLAINTEXT、SSL、SASL_PLAINTEXT 和 SASL_SSL
 */

case class EndPoint(host: String, port: Int, listenerName: ListenerName, securityProtocol: SecurityProtocol) {
  /**
   * 构造完整的监听器连接字符串， 格式为：监听器名称://主机名：端口，比如：PLAINTEXT://kafka-host:9092
   * @return
   */
  ...
  /**
   * clients工程下有一个Java版本的Endpoint类供clients端代码使用，此方法是构造Java版本的Endpoint类实例
   * @return
   */
