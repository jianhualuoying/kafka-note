...
class Partition(val topicPartition: TopicPartition,
  ...
  def makeLeader(partitionState: LeaderAndIsrPartitionState,
    ...
    val (leaderHWIncremented, isNewLeader) = inWriteLock(leaderIsrUpdateLock) {
      ...
      // 更新请求信息中 AR 和 ISR 集合中所有副本对应的 Replica 对象到当前分区的缓存中
      ...
      // 更新 leader epoch的 start offset 为 当前日志的 endOffset，因为是新Leader
      ...
      // 遍历所有的 follower 副本，更新对应副本的相关时间戳信息，包括 lastCaughtUpTime 以及它们的 LastFetchTimems 和 LastFetchLeaderLogEndOffsoffset
      // remoteReplicas 是 updateAssignmentAndIsr 函数中更新的当前分区对应的远程副本
      ...
      // 如果当前 leader 是新选举出来的，则修正 leader 副本的 HW 值，
      if (isNewLeader) {
        // mark local replica as the leader after converting hw
        leaderReplicaIdOpt = Some(localBrokerId)
        // reset log end offset for remote replicas
        // 并重置本地缓存的所有远程副本的相关信息(包括leo值)
        remoteReplicas.foreach { replica =>
      ...
      // Leader 副本会将自己更新后的 LEO 值与 remoteReplicas 中副本的 LEO 值进行比较，来决定是否“抬高”高水位值
  ...
  def makeFollower(partitionState: LeaderAndIsrPartitionState,
                   highWatermarkCheckpoints: OffsetCheckpoints): Boolean = {
    inWriteLock(leaderIsrUpdateLock) {
      // 获取当前分区的leader BrokerId
      val newLeaderBrokerId = partitionState.leader
      val oldLeaderEpoch = leaderEpoch
      // record the epoch of the controller that made the leadership decision. This is useful while updating the isr
      // to maintain the decision maker controller's epoch in the zookeeper path
      // 获取当前分区的controller Epoch，并更新到当前 Broker 缓存中
      controllerEpoch = partitionState.controllerEpoch
      ...
      if (leaderReplicaIdOpt.contains(newLeaderBrokerId) && leaderEpoch == oldLeaderEpoch) {
        ...
        // 重设 Leader 副本的 Broker ID 到分区缓存中，并且返回当前 Broker 成为当前分区的 Follower 副本的成功标志
  ...
  def updateAssignmentAndIsr(assignment: Seq[Int],
    ...
    // assignment是要分配非当前分区的副本，其中，BrokerId和本地的副本localBrokerId不相等的都是远程副本newRemoteReplicas
    val newRemoteReplicas = assignment.filter(_ != localBrokerId)
    // remoteReplicasMap中元素 不在 newRemoteReplicas 中的远程副本，都是不属于当前分区的副本，需要删除掉
  ...
  def appendRecordsToLeader(records: MemoryRecords, origin: AppendOrigin, requiredAcks: Int): LogAppendInfo = {
    val (info, leaderHWIncremented) = inReadLock(leaderIsrUpdateLock) {
      // leaderLogIfLocal是函数调用，判断属于当前分区的日志是否是分区的Leader副本日志？
      leaderLogIfLocal match {
        // leaderLog是的Log对象
        case Some(leaderLog) =>
          val minIsr = leaderLog.config.minInSyncReplicas
