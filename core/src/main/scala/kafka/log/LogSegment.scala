 ...
 *
 * @param log The file records containing log entries 消息日志文件（保存Kafka消息的对象）
 * @param lazyOffsetIndex The offset index 位移索引文件（延迟初始化）
 * @param lazyTimeIndex The timestamp index 时间戳索引文件（延迟初始化）
 * @param txnIndex The transaction index 已中止事务索引文件
 * @param baseOffset A lower bound on the offsets in this segment 日志段对象的起始位移（磁盘上的文件名）
 * @param indexIntervalBytes The approximate number of bytes between entries in the index log.index.interval.bytes 日志段对象新增索引项的频率，
 *                           默认日志段到4kb，会新增一条索引项，再有消息数据会写入新文件。
...
class LogSegment private[log] (val log: FileRecords,
  ...
  def append(largestOffset: Long,
    ...
    if (records.sizeInBytes > 0) {
      ...
      // 确保输入参数最大位移值是合法的
      // 标准就是看它与日志段起始位移的差值是否在整数范围内，即 largestOffset - baseOffset 的值是不是 介于 [0，Int.MAXVALUE] 之间。在极个别的情况下，这个差值可能会越界，这时， append 方法就会抛出异常，阻止后续的消息写入。
      ensureOffsetInRange(largestOffset)

      // append the messages 将内存中的消息对象写入到操作系统的页缓存
      val appendedBytes = log.append(records)
      trace(s"Appended $appendedBytes to ${log.file} at end offset $largestOffset")
      // Update the in memory max timestamp and corresponding offset. 更新日志段的最大时间戳以及最大时间戳所属消息的位移值属性。每个日志段都要保存当前最大时间戳信息和所属消息的位移信息。
      // 两个合起来就是时间戳索引项，它保存了时间戳与消息位移的对应关系
      // Broker 端提供定期删除日志的功能吗？比如我只想保留最近 7 天的日志，没错，当前最大时间戳这个值就是判断的依据；而最大时间戳对应的消息的位移值则用于时间戳索引项。
      if (largestTimestamp > maxTimestampSoFar) {
        maxTimestampSoFar = largestTimestamp
        offsetOfMaxTimestampSoFar = shallowOffsetOfMaxTimestamp
      }
      // append an entry to the index (if needed) append 更新索引项和写入的字节数了，日志段每写入 4KB 数据就要写入一个索引项。当已写入字节数超过了 4KB 之后，
      // append 方法会调用索引对象的 append 方法新增索引项，同时清空已写入字节数，以备下次重新累积计算。
  ...
  def recover(producerStateManager: ProducerStateManager, leaderEpochCache: Option[LeaderEpochFileCache] = None): Int = {
    ...
    try {
      for (batch <- log.batches.asScala) {
        ...
        batch.ensureValid()
        // 该集合中最后一条消息的位移值不能越界，即它与日志段起始位移的差值必须是一个正整数值(0< <Int.MaxValue)
        ensureOffsetInRange(batch.lastOffset)
        // 更新遍历过程中观测到的最大时间戳以及所属消息的位移值。同样，这两个数据用于后续构建索引项。
    ...
    // 遍历执行完成后，Kafka 会将日志段当前总字节数和刚刚累加的已读取字节数进行比较，如果发现前者比后者大，
...
// 在 Scala 语言里，在一个源代码文件中同时定义相同名字的 class 和 object 的用法被称为伴生（Companion）。Class 对象被称为伴生类，
// 它和 Java 中的类是一样的；而 Object 对象是一个单例对象，用于保存一些静态变量或静态方法。如果用 Java 来做类比的话，我们必须要编写两个类才能实现，
// 这两个类也就是 LogSegment 和 LogSegmentUtils。在 Scala 中，你直接使用伴生就可以了。
object LogSegment {
...
// LogFlushStats 结尾有个 Stats，它是做统计用的，主要负责为日志落盘进行计时
