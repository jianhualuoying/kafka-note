...
class ZooKeeperClient(connectString: String,
  ...
  // 这里是创建实际 Zookeeper 连接的地方，ZooKeeper是官方的Zookeeper包中的类
  ...
  def handleRequests[Req <: AsyncRequest](requests: Seq[Req]): Seq[Req#Response] = {
    if (requests.isEmpty)
      ...
      // 这里有多个请求（Seq），用CountDownLatch等待所有请求完成后，一起返回。
      ...
      requests.foreach { request =>
        // 这里用到了信号量控制流量，用读写锁缓解并发
        inFlightRequests.acquire()
        try {
          // inReadLock函数用到了柯里化，小括号中的initializationLock和大括号中的匿名函数都是inReadLock的参数，只不过一个是变量，一个是匿名函数；send函数同理
          ...
          case e: Throwable =>
            // 因为CountDownLatch是局部的，而inFlightRequests是全局的，所以这里只释放了信号量，而读写锁在inReadLock函数中释放
            inFlightRequests.release()
            throw e
        }
      }
  ...
  private[zookeeper] def send[Req <: AsyncRequest](request: Req)(processResponse: Req#Response => Unit): Unit = {
    ...
    // DataCallback、StatCallback、Children2Callback等，都是zk的回调接口，那processResult自然也是这些接口中的方法
    request match {
