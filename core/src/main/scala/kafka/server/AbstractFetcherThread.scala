 ...
 * @param sourceBroker  数据源Broker地址。源 Broker是指此线程（ Follower 副本）要从哪个 Broker （ Leader 副本所在的 Broker ）上读取数据
 * @param failedPartitions 处理过程中出现报错的分区
 * @param fetchBackOffMs 当获取分区数据出错后的等待重试间隔，默认是 Broker 端参数 replica.fetch.backoff.ms 值
 * @param isInterruptible 线程是否允许被中断
 * @param brokerTopicStats Broker端主题监控指标，常见的有 MessagesInPerSec、BytesInPerSec 等
 */
abstract class AbstractFetcherThread(name: String,
  ...
  /**
   * 为指定分区构建对应的 FetchRequest.Builder 对象，而该对象是构建FetchRequest 的核心组件。Kafka 中任何类型的消息读取，都是通过给指定 Broker 发送FetchRequest 请求来完成的。
   * @param partitionMap   // 一组要读取的分区列表，分区是否可读取取决于PartitionFetchState中的状态
   * @return
   */
  ...
  private def maybeFetch(): Unit = {
    val fetchRequestOpt = inLock(partitionMapLock) {
      // 为partitionStates中的分区构造 FetchRequest
  ...
  /**
    ...
    * updateFetchOffsetAndMaybeMarkTruncationComplete方法只有在truncateToEpochEndOffsets和truncateToHighWatermark中结尾执行，
    * 简单来说就是truncate之后，更新标记截断完成，更新partitionStates值，如果它们的offsetTruncationState指示截断已完成，则可能将它们标记为截断已完成
  ...
  private def updateFetchOffsetAndMaybeMarkTruncationComplete(fetchOffsets: Map[TopicPartition, OffsetTruncationState]): Unit = {
    val newStates: Map[TopicPartition, PartitionFetchState] = partitionStates.partitionStateMap.asScala
      .map { case (topicPartition, currentFetchState) =>
        ...
        // 重新构建map key->topicPartition val->maybeTruncationComplete
        (topicPartition, maybeTruncationComplete)
      }
    // 更新 PartitionStates中的map
 ...
 * (1) Truncating its log, for example having recently become a follower 截断中，表明分区副本正在执行截断操作（比如该副本刚刚成为 Follower 副本）
 * (2) Delayed, for example due to an error, where we subsequently back off a bit 被推迟，表明副本获取线程获取数据时出现错误，需要等待一段时间后重试
 * (3) ReadyForFetch, the is the active state where the thread is actively fetching data. 可获取，表明副本获取线程当前能够读取数据
 ...
 * @param delay
 * @param state  分区读取状态，判断分区是否处于可获取、截断中状态比副本状态ReplicaState的判断更加严格
 */
 ...
 * @param offset                位移值
 * @param truncationCompleted   截断完成与否
 */
