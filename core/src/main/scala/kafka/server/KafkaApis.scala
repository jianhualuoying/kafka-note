 ...
 *
 * @param requestChannel  SocketServer组件中的请求通道对象，请求队列提供者
 * @param replicaManager 副本管理器，管理集群所欲副本状态转换
 * @param adminManager  Broker内部管理主题、分区、配置等数据的管理器
 * @param groupCoordinator  消费者组协调器组件，用于维护消费者组的管理
 * @param txnCoordinator  事务管理器组件，实现kafka事务功能
 * @param controller     集群控制器组件，管理与保存元数据
 * @param zkClient       Zookeeper客户端程序，Kafka依赖于该类实现与ZooKeeper交互
 * @param brokerId       broker.id参数值
 * @param config         Kafka配置对象，提供Broker端参数的定义与保存
 * @param metadataCache  元数据缓存对象，保存和更新集群 Broker 间元数据缓存
 * @param metrics
 * @param authorizer
 * @param quotas        配额管理器组件，负责客户端、副本等对象的配额管理
 * @param fetchManager
...
class KafkaApis(val requestChannel: RequestChannel,
  ...
  private def getOrCreateInternalTopic(topic: String, listenerName: ListenerName): MetadataResponseData.MetadataResponseTopic = {
    ...
    // 取集合第一个，因为 kafka.server.MetadataCache.getTopicMetadata 获取的元数据集合  Seq[MetadataResponseTopic]
    ...
    if (findCoordinatorRequest.data.keyType == CoordinatorType.GROUP.id &&
      ...
      val (partition, topicMetadata) = CoordinatorType.forId(findCoordinatorRequest.data.keyType) match {
        case CoordinatorType.GROUP =>
          // 计算分区（key = org.apache.kafka.common.requests.FindCoordinatorRequest.CoordinatorType.GROUP）
          // groupId的hashcode%50（消费者组名哈希值 % 位移主题分区数） ，50是__consumer_offsets的分区个数
          val partition = groupCoordinator.partitionFor(findCoordinatorRequest.data.key)
          // 这里拿到的是 __consumer_offsets 主题所有分区的元数据
      ...
      // createResponse 创建响应，从__consumer_offsets 主题所有分区的元数据 topicMetadata 中找出分区对应的 Coordinator 节点
      // TODO 但是createResponse是一个回调函数，是在发送完请求响应后的回调函数？可是应该通过 createResponse 创建响应才能发送啊
      def createResponse(requestThrottleMs: Int): AbstractResponse = {
        ...
        val responseBody = if (topicMetadata.errorCode != Errors.NONE.code) {
          ...
          // 获取消费者所在分区的leader所在的node （这个节点就是 Coordinator 节点）
  ...
  def handleListGroupsRequest(request: RequestChannel.Request): Unit = {
    ...
    // 通过 GroupCoordinator 组件获取到所有的消费者组信息
  ...
  def handleJoinGroupRequest(request: RequestChannel.Request): Unit = {
    ...
    if (joinGroupRequest.data.groupInstanceId != null && config.interBrokerProtocolVersion < KAFKA_2_3_IV0) {
      ...
      // 决定成员入组参数 requireKnownMemberId，没有配置过 Consumer 端参数 group.instance.id 的话，那么，这个字段的值就是 True，这说明，Kafka 要求消费者成员加入组时，必须要分配好成员 ID。
  ...
  /**
   *
   * 发送 NoOpResponse 类型的 Response 而不受请求通道上限流（throttling）的限制。所谓的 NoOpResponse，
   * 是指 Processor线程取出该类型的 Response 后，不执行真正的 I/O 发送操作
   * @param request
   */
