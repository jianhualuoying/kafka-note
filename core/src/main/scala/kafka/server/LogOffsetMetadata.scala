 ...
 *  1. the message offset 消息位移值（高水位）
 *  2. the base message offset of the located segment 保存该位移值所在日志段的起始位置，该值辅助计算两条消息在物理磁盘文件中位置的差值，即两条消息彼此隔了多少字节。
 * 这个计算有个前提条件，即两条消息必须处在同一个日志段对象上，不能跨日志段对象。否则它们就位于不同的物理文件上，计算这个值就没有意义了。这里的segmentBaseOffset就是用来判断两条消息是否处在同一个日志段中
 *  3. the physical position on the located segment 保存该位移值所在日志段的物理磁盘位置，这个字段在计算两个位移值之间的物理磁盘位置差值时非常有用。你可以想一想，Kafka 什么时候需要计算位置之间的字节数呢？
...
case class LogOffsetMetadata(messageOffset: Long,
  ...
  // 用来判断给定的两个 LogOffsetMetadata 对象是否处于同一个日志段的。判断方法很简单，就是比较两个 LogOffsetMetadata 对象的 segmentBaseOffset 值是否相等
